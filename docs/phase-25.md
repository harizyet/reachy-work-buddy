...
- **reachy-hub** owns intake: the upload endpoint, the live-recording
  hookup to the existing Call Reachy WebRTC session, job creation, and
  status polling. It runs transcription, since STT already lives here
  (Phase 8's rationale in `stt.py` — STT is a hub concern, not core or
  embodiment). Long-form transcription runs in a background worker, not
  inline in the request handler that accepts the upload.
- **companion-core** owns the actual minutes generation: chunked
  summarization via the existing `llm/client.py`/role-routing stack,
  structured extraction of summary/key points/action items, the new
  `MeetingMinutes` store, action-item-to-task integration (reusing the
  Phase 11 tasks store and `task_intent.py`), and sensitivity
  classification/delivery routing for the result (reusing Phase 12's
  provenance/sensitivity model and the ADR 0006 response router). Hub
  forwards the finished transcript text to core once transcription
  completes; core never touches raw audio.
- **clients/operator-ui** adds a Meetings view: upload/start-recording,
  job status, minutes display, and action-item review/confirm.
...
abstraction": a `meeting_jobs` table (Postgres, no new broker/Redis) with a
status column (`uploaded → transcribing → summarizing → complete → failed`),
polled by a single in-process worker loop per hub instance — the same shape
as the existing presence-loop/heartbeat background tasks (Phase 3/22), not
a new pattern. The operator UI polls `GET /meetings/{id}` for status,
mirroring the existing Telegram-poll-health display pattern (Phase 20)
rather than inventing a new one. A crashed/restarted hub must resume or
cleanly fail in-flight jobs, not leave them stuck `transcribing` forever —
test this explicitly (see Exit criteria).
...
Extracted action items are held as unconfirmed candidates attached to the
`MeetingMinutes` record — the "Prepare" tier from `docs/plan.md`'s
permission table (generate preview, no external change yet). The owner
reviews the list in the operator UI and confirms individually or in bulk;
only confirmed items are written to the Phase 11 tasks store, tagged with
their originating meeting for provenance. Editing an item's text before
confirming is allowed; nothing is created from an item the owner didn't see.
...
- `MeetingMinutes` — recording id, summary, key points (list), action items
  (list of `{text, confirmed: bool, task_id: Optional}`), transcript
  reference, sensitivity classification, structured-extraction-failed flag.

New tables via a Phase 23 migration (not `CREATE TABLE IF NOT EXISTS`):
`meeting_jobs`, `meeting_minutes`. No changes to existing tables.
...
- Raw audio is deleted after successful transcription by default
  ...
  transcript/minutes are the durable artifact, governed by Phase 12's
  provenance/sensitivity/expiry model like other memory.
...
1. Confirm Phase 23's migration framework is in place; add the
   `meeting_jobs`/`meeting_minutes` migration.
...
3. Extend `reachy_hub/stt.py`'s usage for long-form audio (verify actual
   ...
   before assuming it fits the background-job budget) and add the
   upload/status/job-worker endpoints.
4. Wire live recording from the existing Call Reachy WebRTC session as a
   second intake path into the same job pipeline.
5. Add core's chunked summarization/extraction pipeline and
   `MeetingMinutes` store, with the retry/fallback behaviour above.
6. Add action-item review/confirm, writing confirmed items into the
   existing Phase 11 tasks store with meeting provenance.
7. Add the operator UI Meetings view (upload/record, status, minutes,
   action-item review) and the cloud-transcript opt-in setting.
8. Apply sensitivity classification and routing to the finished minutes;
...
| Async correctness | A hub restart mid-job does not leave the job stuck; the UI reflects failure or resumption, not silent stall |
