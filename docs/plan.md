...
Implemented foundations; original deliverables and exit criteria are retained
below. Verification scope and historical limits are in
[implementation history](verification/history.md); these rows do not assert
that every physical deployment scenario has passed.

| Phase | Primary deliverable | Original exit criterion |
|---|---|---|
...
| Phase 8 — Modular speech stack | Adapt Silero VAD, faster-whisper, streaming TTS behind provider interfaces. | Modular speech stack works independently of Jarvis. Physical Reachy microphone-to-speaker conversation was not proven here; implementation and physical evidence are recorded in [Phases 24c–24d](phase-24cd.md). |
...
|---|---|---|---|---|
| 19 | Implemented | Owner login, operator dashboard, runtime LLM settings | Real OVMS checks; deployment acceptance remains separate | [History](verification/history.md), [ADR 0016](adr/0016-operator-ui.md) |
| 20 | Implemented | Web chat and Telegram polling health | Browser and real OVMS checks; physical channels matrix remains open | [History](verification/history.md), [ADR 0017](adr/0017-web-chat-channel.md) |
| 21 | Implemented | Local/cloud role routing, fallback and explicit override | Real OVMS and hosted Together AI verified | [History](verification/history.md), [ADR 0018](adr/0018-hybrid-llm-routing.md) |
| 22a | Implemented | Nano backend, outbound WSS substrate, launchers | Live bring-up plus simulator checks; semantic commands still HTTP | [Bring-up](verification/phase-22-bring-up.md) |
| 22b | Partial acceptance | Real motion, audio and camera exercised | Named behaviours owner-accepted; endurance, outage, restore, privacy/consent and channels remain | [Matrix](phase-22-23.md#satisfactory-run-acceptance-matrix), [motion](verification/phase-22b-first-motion-2026-09-23.md) |
| 22c | Partial acceptance | LOCAL camera backend | Real Nano captures succeeded; deliberate fresh-scene acceptance still owed | [Camera](verification/phase-22b-camera-2026-09-24.md) |
| 23 | Implemented | Migrations, SecretStore, Google Accounts | Fixture/browser/database checks; live Google, audience and physical repeat open | [Accounts](verification/phase-23-accounts-2026-09-23.md), [release gate](phase-22-23.md#phase-23-acceptance-and-production-release-gate) |
| 23b | Implemented | Desktop OAuth loopback helper | Fixture verified; live helper consent and inherited 23 gates open | [Desktop OAuth](verification/phase-23b-desktop-oauth-2026-09-24.md) |
| 24a | Implemented | Search grounding, providers and settings | Real SearXNG and hosted rotation evidence; production acceptance open, quality/search fixes in 24e | [Search and addenda](verification/phase-24a-search-assisted-2026-09-24.md) |
...
| 24e | In progress | Adaptive turns, search/correctness, deferred 24d rows, Nano diagnostics | Adaptive end of turn implemented, not deployed or physically accepted; hardware conversation-path subset must PASS before 25 | [Scope and exit](phase-24e.md#exit-criteria), [25 prerequisites](phase-24e.md#prerequisites-for-phase-25) |
| 24f | In progress; blocked on hardware | Motion conformance, verified startup home, local conversation animation | REST/SDK parity measured; robot misses commanded poses (suspected mechanical fault). Motion owner implemented, off by default. Enabled changes must pass before Phase 25 uses them | [Scope and exit](phase-24f.md#delivery-sequence-and-exit) |
...
Start with [Phase 24e's implementation sequence](phase-24e.md#implementation-sequence).
Its [conversation-path prerequisites](phase-24e.md#prerequisites-for-phase-25)
must pass on hardware before Phase 25 begins. Outstanding platform and
Google acceptance can proceed independently. [Phase 24f](phase-24f.md) plans
motion conformance and conversational embodiment after the 24e baseline; its
[dependency rule](phase-24f.md#order-and-dependencies) applies to enabled motion
changes used by Phase 25. Use the ledger above and
